...
class Forecasts(MetricBuilder):
    ...
    def metric_columns(self) -> List:
        return list(FORECASTS_WEIGHTS.keys())
    ...
    def quantiles(self):
        ...
        for key in ALLOWED_GROUPS:
            g = self.data[[key] + list(self.metric_columns)].groupby(key)
    ...
    def initialize_weights(self):
        self.weights = pd.DataFrame(
            data={k: [v] * self.data.shape[0] for k, v in FORECASTS_WEIGHTS.items()},
    ...
    def calculate_weights(self):
        ...
        w["Top Analyst Price Target Upside (%)"] *= (
            1 + self.data["Top Analyst Count"] / self.data["Top Analyst Count"].max()
        )
